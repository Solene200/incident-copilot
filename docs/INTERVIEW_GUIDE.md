...
IncidentCopilot 是一个证据驱动的智能故障诊断项目。我用 LangGraph 把已校验事故上下文、
调查计划、多源并行取证、假设生成、充分性判断、有限追加调查、报告和人工审核建模为可恢复
工作流。系统默认完全离线，使用脱敏 Fixture、Fake Model 和 Fake Embedding；payment-only
作品集演示可以把 synthetic OpenTelemetry 指标经 Collector 和 Prometheus 接入同一
`MetricsProvider`，最终报告中的关键结论回指 Evidence ID 和 citation。

面试时必须先说明产品边界：API 接收自然语言描述，但单个 primary service 与带时区时间窗
由调用方结构化提供；当前不会仅从 raw query 自动解析这些字段。

| 状态 | 可陈述能力 |
| --- | --- |
| Current | Fixture/Fake 默认调查、单 primary service、FastAPI/SSE/HITL、双工具预算、可解析的仓库内 Citation |
| Current（窄场景） | payment synthetic metrics 的 Prometheus HTTP 链路 |
| Experimental | PgVectorStore 参数化 SQL Adapter；非默认链路、无 live DB RAG 验收 |
| Target | raw-query parser、真实模型/Embedding、多服务联合调查、持久业务/Evidence Store、生产安全与分布式运行 |
...
默认 Fake Embedding 是确定性 signed-hash，只用于验证管线、排序稳定性和无网络测试，
不应声称有真实语义质量。pgvector Adapter 是 **Experimental**：只有参数化 SQL 契约和
recording fake 测试，默认演示仍使用内存向量索引。
...
当前只实现了 payment 场景的 Prometheus metrics 窄切片：
...
不能声称 checkout DNS 或 inventory cache 已有 live Prometheus 证明；这两个场景目前只在
Fixture Graph 与离线 Evaluation 中验证。
