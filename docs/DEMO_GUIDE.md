...
状态边界：Fixture/API 演示是 **Current**；Prometheus 是 **Current（payment-only）**；
`PgVectorStore` 是 **Experimental** 且不参与本指南的 RAG；DNS/cache live metrics、真实
LLM/Embedding 和全生产栈是 **Target**。
...
| PostgreSQL/pgvector 镜像 | `pgvector/pgvector:0.8.5-pg18-trixie` | 只用于 LangGraph checkpoint；不表示 PgVectorStore 已接入 RAG |
...
当前 live 场景只验证 `payment-service`；checkout DNS 与 inventory cache 只在 Fixture
Evaluation 中覆盖，没有等价的 Prometheus mapping/Compose smoke evidence：
...
Evidence ID、report ID 和时间每次会变化，不应写死到断言或演示话术中。脚本要求跨两个 scrape 周期连续读到真实序列后才启动 Graph，避免 Prometheus 健康但首批指标尚未稳定的冷启动竞态；60 秒内不能满足条件会以非零状态失败，不会伪造回退结果。
...
以下记录是历史功能验证，不自动代表当前 commit 已重新执行 Compose：

| 日期 | 被验证代码 | 环境 | 证据位置 | 当前解释 |
| --- | --- | --- | --- | --- |
| 2026-07-18 | `9249b8c`（Compose/runtime 实现提交） | Windows + Docker Desktop；固定镜像见第 2 节 | `docs/PROGRESS.md` 的 Phase 7“真实容器验收” | Historical；证明当时 payment-only synthetic metrics 链路可运行，不是当前提交的自动复验 |
...
Batch D 的默认质量门禁不重跑 Docker Compose；若要对当前提交宣称 live 验证，必须重新执行
第 4、5 节命令，并把日期、commit、环境和输出 artifact 一并记录。
